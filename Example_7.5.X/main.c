...
unsigned char response, x, data = ' ', d;
...
unsigned char SD_init(void)
{
    ...
    do{
        ReadSPI_();                                                       //
        count++;
      }while((response!= 0x00)&&(response!= 0x01));
    i=0;
    do{
        Command(55,  0 , 0xFF);
        do{
           ReadSPI_();                                                   //
          }while((response!=0x00)&&(response!=0x01)&&(response!=0x05));
          Command(1,  0 , 0xFF);
            do{
              ReadSPI_();                                                 //
              }while((response!= 0x00)&&(response!= 0x01));
...
{
 do{
 Write_str("\n\rPlease select a file operation?\t\tWrite    - w\tRead     - r\n\r\t\t\t\t\tDelete   - d\tAppend   - a\n\r\t\t\t\t\tNew Dir  - n\tFormat   - f\n\r\t\t\t\t\tOpen Dir - x\n\r");
 c = ReadUART();
 switch(c)
    {
         case 'w' :
...
void main()
{
   ...
   if(!SD_init())
    ...
    Write_str("\n\rCARD initialization failed..!");                                                                            // Initialise File IO Library
